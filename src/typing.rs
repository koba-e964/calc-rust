use crate::ast::{ty_of_ast, FunDec, Op, Type, TypedAST, TypedFunDec, AST};
...
fn expect_type(
    ...
    if ty_of_ast(&tast) != *expected_type {
        panic!(
            "type error in expression {:?}: expected: {:?}, but got: {:?}\n{}",
            ast,
            expected_type,
            ty_of_ast(&tast),
            msg
...
fn f_sub(fundecs: &[FunDec], ast: &AST, env: &mut HashMap<String, Type>) -> TypedAST {
    match *ast {
        ...
        AST::FunApp(ref f, ref es) => {
            ...
            if n != m {
                panic!(
                    "The number of arguments of {} is wrong. (expected: {}, got: {})",
                    f.clone(),
                    m,
                    n
                );
            ...
            let typed_es = (0..n)
                .map(|i| expect_type(fundecs, &es[i], env, &sign.0[i], "Argument type differs."))
                .collect::<Vec<_>>();
...
fn get_signature(fundecs: &[FunDec], name: &str) -> (Vec<Type>, Type) {
    ...
    (
        fundec.1.iter().map(|param| param.1.clone()).collect(),
        fundec.2.clone(),
    )
}
...
mod tests {
    use super::*;
    use crate::{ast, parse, typing};
    fn typing_ast(ast: &AST) -> TypedAST {
        typing::f(&Vec::new(), ast).1
    }
    #[test]
    fn operations_test() {
        let ast1 = AST::OpNode(Op::Sub, Box::new(AST::Num(7)), Box::new(AST::Num(4)));
        assert_eq!(ast::ty_of_ast(&typing_ast(&ast1)), Type::Int);
        let ast2 = AST::OpNode(Op::Div, Box::new(AST::Num(20)), Box::new(AST::Num(4)));
        assert_eq!(ast::ty_of_ast(&typing_ast(&ast2)), Type::Int);
    }
    #[test]
    fn letex_test() {
        let ast1 = parse::parse("let x = 4 in x + x");
        assert_eq!(ast::ty_of_ast(&typing_ast(&ast1.1)), Type::Int);
        let ast2 = parse::parse("let x = 4 in let x = 3 in x + x");
        assert_eq!(ast::ty_of_ast(&typing_ast(&ast2.1)), Type::Int);
        let ast3 = parse::parse("let x = 4 in (let x = 3 in x) + x");
        assert_eq!(ast::ty_of_ast(&typing_ast(&ast3.1)), Type::Int);
    }
    #[test]
    fn fundec_test() {
